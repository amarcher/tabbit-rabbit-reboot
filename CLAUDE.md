...
- **Anthropic vision API**: Only accepts image/jpeg, image/png, image/gif, image/webp, with a ~5MB base64 payload cap. Mismatched bytes/media_type (e.g. HEIC labeled as JPEG) or oversized payloads fail with `invalid_request_error: Could not process image`. All normalization happens **client-side**:
  - **Web** (`src/utils/imageNormalization.ts`): canvas re-encode to JPEG @ ≤1600px. HEIC inputs (detected via MIME type or `.heic`/`.heif` extension — iOS Photos' default, and Chrome's `<img>` can't decode it) are routed through **heic2any**, a libheif WASM decoder that's **dynamic-imported** so the ~800KB WASM bundle only loads when a HEIC file is actually picked. Don't try to server-side normalize with `sharp` — its prebuilt binaries omit HEIC decode due to libheif licensing, and bundling `heic-convert` + `sharp` blows past Vercel's 50MB serverless function limit.
