...
- **UI** (`src/components/VoiceAssignmentModal.tsx`): Three-phase modal (review → processing → confirm). Review phase shows a textarea for typing/dictating who had what. Accepts optional `initialTranscript` prop. No speech recognition — relies on browser dictation or manual text entry. The confirm phase shows a fraction bar + label when an item is split between multiple people (e.g., ½ bar for 50/50 splits).
