...
- **Anthropic vision API**: Only accepts image/jpeg, image/png, image/gif, image/webp, with a ~5MB base64 payload cap. Mismatched bytes/media_type (e.g. HEIC labeled as JPEG) or oversized payloads fail with `invalid_request_error: Could not process image`. Both platforms re-encode every user-picked image to JPEG @ ≤1600px longest edge before sending: web via canvas (`src/utils/imageNormalization.ts`), mobile via `expo-image-manipulator` (inline in `mobile/app/tab/[tabId].tsx`). iOS Safari and the iOS Photos library can both hand us HEIC directly, so this normalization is load-bearing.
