...
- **UI**: Inline input bar in `TabEditor.tsx` (textarea + Cancel/Commit buttons, no modal overlay) for typing/dictating who had what. On commit, `VoiceAssignmentModal.tsx` opens for processing → confirm only. No speech recognition — relies on browser dictation or manual text entry. The confirm phase shows a fraction bar + label when an item is split between multiple people (e.g., ½ bar for 50/50 splits). "Try Again" dismisses the modal and reopens the inline input bar with text preserved.
...
- **Anthropic vision API**: Only accepts image/jpeg, image/png, image/gif, image/webp, with a ~5MB base64 payload cap. Mismatched bytes/media_type (e.g. HEIC labeled as JPEG) or oversized payloads fail with `invalid_request_error: Could not process image`. All normalization happens **client-side**:
  - **Web** (`src/utils/imageNormalization.ts`): canvas re-encode to JPEG @ ≤1600px. HEIC inputs (detected via MIME type or `.heic`/`.heif` extension — iOS Photos' default, and Chrome's `<img>` can't decode it) are routed through **heic-to**, a libheif WASM decoder that's **dynamic-imported** so the WASM bundle only loads when a HEIC file is actually picked. **Do not use heic2any** — it's unmaintained (last published years ago) and ships with an older libheif that rejects newer iPhone HEIC variants (HEVC Main 10) with `ERR_LIBHEIF format not supported`. Also don't try to server-side normalize with `sharp` — its prebuilt binaries omit HEIC decode due to libheif licensing, and bundling `heic-convert` + `sharp` blows past Vercel's 50MB serverless function limit.
  - **Mobile** (`mobile/app/tab/[tabId].tsx`): `expo-image-manipulator` normalizes to JPEG @ ≤1600px pre-upload. It uses iOS's native image frameworks, which decode HEIC natively.
